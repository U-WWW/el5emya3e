...
export default function StudentDashboard({ onLogout, currentTheme = 'khemiai_dark', onThemeChange }: StudentDashboardProps) {
  ...
  // Dynamic Hero Banner & Teacher Settings (Science & Chemistry Defaults)
  const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1532094349884-543bc11b234d?q=80&w=2070&auto=format&fit=crop';
  const DEFAULT_TEACHER_IMAGE = '/teacher.png';

  const SCIENCE_HERO_DEFAULTS = {
    imageUrl: DEFAULT_HERO_IMAGE,
    badgeText: 'منصة الخيميائي للعلوم والكيمياء 🧪',
    mainTitle: 'مرحباً بك في <span class="text-cyan-400">عالم الكيمياء والعلوم</span>',
  ...
  const [chatMessages, setChatMessages] = useState<any[]>([
    { role: 'model', content: 'أهلاً بك يا بطل الكيمياء والعلوم! أنا "المساعد الذكي للخيميائي". كيف يمكنني مساعدتك اليوم في فهم الدروس، حل المعادلات التفاعلية، والشرح العلمي المفصل؟ 🧪✨' }
  ]);
    ...
    try {
      ...
      if (!studentSnap.empty) {
        ...
        if (data.is_banned) {
          setStudent(null);
          localStorage.removeItem('jamal_student');
          alert('🚨 تم حظر هذا الحساب من المنصة لاستخدام كلمات غير لائقة! يرجى مراجعة الأستاذ الخيميائي.');
          setLoading(false);
          return;
        }
      }
  ...
  const getBadgeAndDescription = (score: number) => {
    if (score >= 121) return { badge: 'إمبراطور الكيمياء 👑', color: 'text-amber-400 border-amber-400', desc: 'سيد مختبر العلوم والدرجة الكاملة بلا منازع!' };
    if (score >= 76) return { badge: 'العالم الخيميائي 🧬', color: 'text-rose-400 border-rose-400', desc: 'صاحب النظريات والتفاعلات الكيميائية الفائقة.' };
    if (score >= 41) return { badge: 'كيميائي المحاليل ⚗️', color: 'text-sky-400 border-cyan-400', desc: 'محلل التفاعلات والعناصر المركبة بذكاء.' };
    if (score >= 16) return { badge: 'باحث المختبر 🧪', color: 'text-amber-500 border-sky-400', desc: 'مكتشف المركبات والمعادلات الكيميائية بمهارة.' };
    return { badge: 'مستكشف الذرات ⚛️', color: 'text-amber-500 border-amber-500', desc: 'مستكشف جديد يبدأ أولى خطواته في عالم العلوم الكيميائي.' };
  };
  ...
  if (loading && !student) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-stone-950">
        ...
        <p className="text-xl font-bold tracking-widest text-amber-500 uppercase">Verifying Alliance Credentials...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-12 relative z-10">
        <div className="w-full max-w-lg bg-stone-950/80 backdrop-blur-2xl border border-white/10 shadow-2xl p-8 md:p-10 rounded-3xl relative overflow-hidden">
          ...
          <div className="text-center mb-8 relative">
            <div className="inline-block bg-cyan-500/20 border border-cyan-500/30 px-6 py-2 rounded-full mb-6 backdrop-blur-md">
              <span className="text-xs font-bold text-cyan-400 tracking-widest">
                منصة الأستاذ الخيميائي للعلوم والكيمياء 🧪
              </span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black text-white tracking-tight mb-3">
              بوابة الخيميائيين الأبطال
            </h2>
            <p className="text-sm font-medium text-stone-300">
              ادخل بوابات الأكاديمية وصقل مهاراتك العلمية والكيميائية مع الأستاذ الخيميائي
            </p>
          </div>


          {/* Login vs Register Tabs Selector */}
          <div className="grid grid-cols-2 gap-2 bg-stone-900 border border-stone-800 p-1 rounded-xl mb-6">
            <button
              onClick={() => {
                setAuthMode('login');
                setLoginError('');
            ...
            <form onSubmit={handleLogin} className="space-y-5 text-right">
              ...
              >
                {isSubmittingLogin ? (
                  ...
                  <>
                    الدخول للمنصة <BookOpen className="w-5 h-5 text-slate-950" />
                  </>
                )}
              </button>
            </form>
            ...
            <form onSubmit={handleRegister} className="space-y-5 text-right">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <User className="absolute left-4 top-4 text-stone-500 w-5 h-5" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <Smartphone className="absolute left-4 top-4 text-stone-500 w-5 h-5" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <GraduationCap className="absolute left-4 top-4 text-stone-500 w-5 h-5" />
                  </div>
                </div>
              ...
              >
                {isSubmittingLogin ? (
                  ...
                  <>
                    إنشاء حساب جديد <BookOpen className="w-5 h-5 text-slate-950" />
                  </>
                )}
              </button>
            </form>
        ...
        <AnimatePresence>
          {newlyRegisteredStudent && (
            ...
            >
              <div className="w-full max-w-xl bg-stone-900 border border-amber-600/50 shadow-2xl rounded-2xl p-6 md:p-8 relative text-center">
                <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-amber-700 text-stone-100 font-sans font-black px-6 py-2 rounded-full text-lg shadow-md border border-amber-500/50">
                  مرحباً بك في المنصة
                </div>

                <div className="mt-6 mb-6">
                  <div className="inline-flex p-4 rounded-full bg-amber-900/30 border border-amber-600/50 text-amber-500 mb-4">
                    <Trophy className="w-12 h-12" />
                  </div>
                  <h3 className="text-3xl font-black text-stone-100 tracking-tight">
                    مبارك لك يا بطل العلوم والكيمياء!
                  </h3>
                  <p className="text-sm font-bold text-stone-400 mt-1">
                    تم إنشاء هويتك في منصة الخيميائي للعلوم والكيمياء. يرجى الاحتفاظ ببيانات البوابة التالية بعناية فائقة للدخول مستقبلاً:
                  </p>
                </div>

                {/* THE HISTORICAL DOCUMENT DISPLAY */}
                <div className="bg-[#fdf6e3] text-stone-900 p-6 rounded-xl border border-amber-900/20 shadow-inner text-right space-y-4 mb-6 relative overflow-hidden">
                  <div className="absolute top-2 left-2 text-stone-900 opacity-5 font-sans font-black text-6xl select-none uppercase tracking-tighter">
                    CHEMISTRY
                  </div>
                  
                  <div className="border-b border-stone-900/20 pb-2 mb-2 flex items-center justify-between">
                    <span className="font-sans font-black text-xs uppercase tracking-wider text-amber-900">بطاقة هوية طالب الخيميائي</span>
                    <span className="text-xs font-black bg-amber-900 text-amber-50 px-2 py-0.5 rounded">مستوى 1</span>
                  </div>

                  <div className="space-y-2">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-wider block text-stone-500">اسم الطالب (الاسم)</span>
                      <span className="text-lg font-black block text-stone-900">{newlyRegisteredStudent.name}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-4 pt-2">
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-wider block text-stone-500">كود الدخول الفريد</span>
                        <span className="text-xl font-mono font-black block text-amber-700 tracking-wider">
                          {newlyRegisteredStudent.code}
                        </span>
                      </div>
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-wider block text-stone-500">كلمة المرور</span>
                        <span className="text-xl font-mono font-black block text-stone-900 tracking-wider">
                          {newlyRegisteredStudent.password}
                        </span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 pt-2 border-t border-stone-900/10">
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-wider block text-stone-500">الصف الدراسي</span>
                        <span className="text-xs font-black block text-stone-800">{newlyRegisteredStudent.className}</span>
                      </div>
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-wider block text-stone-500">المجموعة</span>
                        <span className="text-xs font-black block text-stone-800">{newlyRegisteredStudent.groupName}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className="flex-1 bg-stone-800 hover:bg-stone-700 border border-stone-700 text-stone-100 font-black py-3 rounded-xl transition text-sm"
                  >
                    نسخ البيانات 📋
                  </button>
                  <button
  ...
  return (
    <div className="min-h-screen text-stone-100 flex flex-col relative z-10 font-sans">
      ...
      {/* Elegant Header */}
      <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-3xl border-b border-cyan-900/50 shadow-2xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20 md:h-24">
            <div className="flex items-center gap-6">
              {/* Logo / Brand */}
              <div className="flex flex-col">
                <span className="font-black text-xl md:text-2xl tracking-widest text-cyan-400 uppercase drop-shadow-[0_0_12px_rgba(34,211,238,0.5)]">
                  EL-KHEMIAI
                </span>
                <span className="text-[10px] md:text-xs font-bold text-cyan-300/80 tracking-widest uppercase">
                  Science & Chemistry
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center gap-2 bg-slate-900/80 border border-cyan-800/50 px-4 py-2 rounded-xl">
        ...
        <div className="flex justify-around items-center p-2 overflow-x-auto">
          {[
            ...
            return (
              ...
              >
                <Icon className={`w-6 h-6 mb-1 ${isActive ? 'drop-shadow-[0_0_8px_rgba(6,182,212,0.8)]' : ''}`} />
          ...
          <div className="lg:col-span-4 space-y-6 order-2 lg:order-1">
            ...
            <div className="bg-stone-900/80 backdrop-blur-md border border-stone-800 rounded-3xl p-8 relative overflow-hidden shadow-2xl group">
              ...
              <div className="text-center relative z-10">
                ...
                <h3 className="text-2xl font-black tracking-tight text-white mb-2">
                  {teacherSettings.name || 'الأستاذ الخيميائي'}
                </h3>
                <p className="text-sm font-bold text-cyan-400/80 mb-6">
                  {teacherSettings.subtitle || 'خبير تدريس مادة العلوم والكيمياء'}
                </p>
                
                {/* Motivational Quote */}
                <div className="bg-stone-950/50 border border-stone-800 p-5 rounded-2xl text-center relative">
                  <span className="absolute -top-3 right-4 text-3xl text-cyan-900">"</span>
                  <p className="text-xs font-bold text-stone-300 leading-relaxed text-right relative z-10">
                    {teacherSettings.quote || 'الكيمياء ليست مجرد معادلات تُحفظ، بل هي سر فهم الكون وعجائب الطبيعة من حولنا.'}
                  </p>
                </div>
            ...
            <div className="bg-stone-900/80 backdrop-blur-md border border-stone-800 rounded-3xl p-6 space-y-6 shadow-xl">
              ...
              <div className="flex items-center gap-5 bg-stone-950/50 border border-stone-800 p-4 rounded-2xl">
                ...
                <div>
                  <span className="text-xs font-black text-cyan-400 block mb-1">الرمز العلمي الحالي</span>
                  <span className="text-base font-black text-white block">
                    {selectedAvatar === 'ironman' ? 'د. أحمد زويل (رائد الفيمتو) ⚛️' :
                     selectedAvatar === 'captainamerica' ? 'ديمتري مندليف (الجدول الدوري) 🔬' :
                     selectedAvatar === 'thor' ? 'ماري كوري (عالمة الكيمياء) ⚗️' :
                     selectedAvatar === 'blackwidow' ? 'ابن الهيثم (رائد العلوم) 🧬' :
                     selectedAvatar === 'hulk' ? 'ألبيرت أينشتاين (الكيمياء النظرية) 💥' :
                     selectedAvatar === 'doctorstrange' ? 'جون دالتون (النظرية الذرية) 🔮' :
                     'جابر بن حيان (أستاذ الكيمياء) 🧪'}
                  </span>
                </div>
              </div>
            </div>
                ...
                <div className="bg-stone-900 border border-stone-800 rounded-2xl h-[65vh] flex flex-col overflow-hidden">
                  ...
                  <div className="bg-stone-950 p-4 border-b border-gray-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-md font-black text-stone-100 block">المساعد الكيميائي الذكي 🧪</span>
                        <span className="text-xs font-bold text-amber-400 block mt-0.5">مساعد العلوم والكيمياء لمنصة الخيميائي</span>
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === 'profile' && (
                <div className="space-y-8">
                  ...
                  <div className="bg-stone-900 border border-stone-800 shadow-xl p-6 rounded-2xl">
                    <div className="border-b-2 border-gray-800 pb-3 mb-4 text-right">
                      <h4 className="font-sans font-black text-stone-100 text-xl tracking-wider">
                        اختر رمزك المفضل من علماء الكيمياء والعلوم 🧪
                      </h4>
                      <p className="text-xs font-bold text-gray-500 mt-1">
                        تغيير الرمز يمنحك طابعاً بصرياً وعلمياً فريداً في المنصة وصحيفة الإنجازات
                      </p>
                    </div>

                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        { id: 'spiderman', emoji: '🧪', nameAr: 'جابر بن حيان', color: 'border-amber-500 hover:border-amber-400 bg-amber-950/20' },
                        { id: 'ironman', emoji: '⚛️', nameAr: 'أحمد زويل', color: 'border-cyan-500 hover:border-cyan-400 bg-cyan-950/20' },
                        { id: 'captainamerica', emoji: '🔬', nameAr: 'ديمتري مندليف', color: 'border-blue-500 hover:border-blue-400 bg-blue-950/20' },
                        { id: 'thor', emoji: '⚗️', nameAr: 'ماري كوري', color: 'border-emerald-500 hover:border-emerald-400 bg-emerald-950/20' },
                        { id: 'blackwidow', emoji: '🧬', nameAr: 'ابن الهيثم', color: 'border-purple-500 hover:border-purple-400 bg-purple-950/20' },
                        { id: 'hulk', emoji: '💥', nameAr: 'ألبرت أينشتاين', color: 'border-rose-500 hover:border-rose-400 bg-rose-950/20' },
                        { id: 'doctorstrange', emoji: '🔮', nameAr: 'جون دالتون', color: 'border-amber-600 hover:border-amber-500 bg-amber-950/20' }
                      ].map(hero => (
                        <button
                          key={hero.id}
                  ...
                  <div className="bg-stone-900 border border-stone-800 shadow-xl p-6 rounded-2xl lg:hidden">
                    <div className="border-b-2 border-gray-800 pb-3 mb-4 text-right">
                      <h4 className="font-sans font-black text-stone-100 text-xl tracking-wider">
                        اختر الطابع البصري للمختبر (Theme)
                      </h4>
                      <p className="text-xs font-bold text-gray-500 mt-1">
                        اختر السمة البصرية والخلفية العلمية المناسبة لدراستك
                      </p>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <button onClick={() => onThemeChange?.('khemiai_dark')} className={`p-4 rounded-xl border-2 font-black transition flex flex-col justify-center items-center gap-2 ${currentTheme === 'khemiai_dark' ? 'border-cyan-500 bg-cyan-500/20 text-cyan-400' : 'border-stone-800 bg-stone-900 hover:bg-stone-800 text-stone-500'}`}><Palette className="w-8 h-8"/>المختبر الكيميائي</button>
                      <button onClick={() => onThemeChange?.('atomic_glow')} className={`p-4 rounded-xl border-2 font-black transition flex flex-col justify-center items-center gap-2 ${currentTheme === 'atomic_glow' ? 'border-amber-500 bg-amber-500/20 text-amber-400' : 'border-stone-800 bg-stone-900 hover:bg-stone-800 text-stone-500'}`}><Sparkles className="w-8 h-8"/>التفاعل الذري</button>
                      <button onClick={() => onThemeChange?.('deep_emerald')} className={`p-4 rounded-xl border-2 font-black transition flex flex-col justify-center items-center gap-2 ${currentTheme === 'deep_emerald' ? 'border-emerald-500 bg-emerald-500/20 text-emerald-400' : 'border-stone-800 bg-stone-900 hover:bg-stone-800 text-stone-500'}`}><Cpu className="w-8 h-8"/>الزمرد الكيميائي</button>
                    </div>
                  </div>

                  {/* CHEMISTRY STATS GRID */}
                  <div className="bg-stone-900 border border-stone-800 shadow-xl p-6 rounded-2xl">
                    <div className="border-b-2 border-gray-800 pb-3 mb-6 text-right">
                      <span className="text-xs font-black bg-cyan-950 text-cyan-400 border border-cyan-800 px-3 py-1.5 rounded uppercase font-mono inline-block">
                        LEVEL 1 CHEMIST
                      </span>
                      <h4 className="font-sans font-black text-stone-100 text-xl tracking-wider mt-2">
                        مؤشرات تفوقك الكيميائي الستة 📊
                      </h4>
                      <p className="text-xs font-bold text-gray-500">
                        مستوى تفوقك الأكاديمي يتم حسابه بدقة بناءً على تفاعلك وأدائك في اختبارات وتجارب العلوم والكيمياء
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                      {[
                        {
                          name: 'التحليل الكيميائي وفهم التفاعلات 🧪',
                          pct: Math.min(25 + (student.totalScore || 0) * 1.5, 98),
                          color: 'bg-cyan-600',
                          icon: '🧪'
                        },
                        {
                          name: 'حفظ المعادلة والرموز الكيميائية ⚛️',
                        ...
                        {
                          name: 'الالتزام والمواظبة على الدروس 🛡️',
                          pct: Math.min(45 + studentResults.length * 8, 92),
                          color: 'bg-emerald-500',
                          icon: '🛡️'
                        },
                        {
                          name: 'المشاركة الفعالة في الشات العلمي 💬',
                          pct: 85,
                          color: 'bg-purple-500',
                          icon: '💬'
                        },
                        {
                          name: 'نسبة التفوق الكيميائي العام 🏆',
                  ...
                  <div className="bg-stone-900 border border-stone-800 shadow-xl p-6 rounded-2xl">
                    <div className="border-b-2 border-gray-800 pb-3 mb-6 text-right">
                      <h4 className="font-sans font-black text-stone-100 text-xl tracking-wider">
                        أرشيف الاختبارات (سجل درجاتك واختباراتك العلمية السابقة)
                      </h4>
                      <p className="text-xs font-bold text-gray-500 mt-1">
                        جميع نتائج الاختبارات التي سلمتها بنجاح للأستاذ الخيميائي
                      </p>
                    </div>
                  ...
                  <div className="bg-stone-950/40 border-b border-gray-950 p-4 md:p-5 flex flex-col md:flex-row justify-between items-center gap-3">
                    ...
                    <div className="bg-rose-950/30 border border-rose-900/60 p-2.5 rounded-xl text-right max-w-sm">
                      <p className="text-[10px] md:text-xs font-bold text-cyan-300 leading-relaxed">
                        ⚠️ **نظام الحماية والأمان:** الشات مراقب ذاتياً. أي محاولة لكتابة كلمة بذيئة أو خارجة تؤدي لحظر الحساب تلقائياً وفورياً وإبلاغ الأستاذ الخيميائي.
                      </p>
                    </div>
                  </div>
